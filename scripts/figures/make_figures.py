...
def fig1_ablation():
    ...
    ax.set_title("图4-1 因果行为特征三层消融（每层增量单独可测）", fontsize=11)
    fig.tight_layout()
...
def fig2_counterfactual_waterfall():
    ...
    ax.set_title(f"图4-2 被标记案例的反事实瀑布（基础分 {case['base_score']:.3f}，干预=置回普通账户水平）", fontsize=10.5)
    fig.tight_layout()
...
def fig3_frontier():
    ...
    ax1.set_title("图4-3 精度-可归因性权衡前沿", fontsize=11)
    fig.tight_layout()
...
def fig4_shap():
    ...
    ax.set_title("图4-4 全局特征重要性 Top-12（全部具备反洗钱业务语义）", fontsize=11)
    fig.tight_layout()
...
def fig5_gnn_diagnosis():
    ...
    ax.set_ylabel("测试集 AUPRC")
    ax.set_title("图4-5 端到端图神经网络七配置诊断链（种子42）", fontsize=11)
    fig.tight_layout()
...
def fig6_leakage_forensics():
    ...
    ax.set_title("图4-6 泄漏法医 2×2：协议缺陷的量化虚高", fontsize=11)
    fig.tight_layout()
